...
public class ITClubEdicionController {
	...
	public void la_peticion_de_edicion_club_deberia_devolver_la_vista_del_formulario_de_edicion_de_club() throws Exception {
		...
		// WHEN
		ResultActions respuesta = mockMvc.perform(peticionEdicionClub);
		
		// THEN
		respuesta.andExpect(status().isOk())
			.andExpect(view().name(ClubController.VISTA_ADMIN_CLUBS_EDICION))
			.andExpect(forwardedUrl("/WEB-INF/jsp/" + ClubController.VISTA_ADMIN_CLUBS_EDICION + ".jsp"))
			.andExpect(request().attribute(ClubController.REQUEST_ATTRIBUTE_CLUB_FORM, clubFormEsperado));
